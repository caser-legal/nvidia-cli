...
export class ToolOrchestrator {
  ...
  async selectTools(task: string, context?: string): Promise<string[]> {
    const toolDefs = Array.from(this.tools.values()).map(t => ({ name: t.name, description: t.description }));
...
`;
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(selected)) return selected.filter(name => this.tools.has(name));
        if (selected.tools && Array.isArray(selected.tools)) return selected.tools.filter((name: string) => this.tools.has(name));
      } catch {
      ...
      log.error("Tool selection error", { error: String(error) });
      return [];
    }
  }
}
